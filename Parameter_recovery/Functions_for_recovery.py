...
#Start the likelihood estimation process: summed_logL = log(L(parameter set|data))
    ...
    #Define the variables that are important for the likelihood estimation process
        ...
        PEs = np.array([df.loc[trial , "PE_1"] , df.loc[trial , "PE_2"] , df.loc[trial , "PE_3"]])
        LPs = np.array([df.loc[trial , "LP_1"] , df.loc[trial , "LP_2"] , df.loc[trial , "LP_3"]])
        Novs = np.array([df.loc[trial , "Nov_1"] , df.loc[trial , "Nov_2"] , df.loc[trial , "Nov_3"]])
...
if __name__ == '__main__':
    ...
    for file in filtered_filelist:
        ...
        estimated_data.loc[idx , ["Current_PE", "Current_LP" , "Current_Nov"]] = [df.loc[0 , "weight_PE"] , df.loc[0 , "weight_LP"] , df.loc[0 , "weight_Nov"]]
