...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 17 08:57:38 2026
...
def stay_leave_decision(boundaries , current_task , RAs):
    ...
    if current_task == 1: 
        ...
        probability = sigmoid(RAs[0] , 1 , boundaries[0])
        ...
        increment = sigmoid(RAs[1] , 1 , boundaries[1])
        ...
        decrement_stay = sigmoid(RAs[1] , 1 , boundaries[2])
        
        #So now we have a list with 3 probabilities; a prob to decrement, one to stay and one to increment 
        stay_leave = [1 - decrement_stay , decrement_stay , increment]
    
    else: 
        #Get the normalized value for the decision
        probability = sigmoid(RAs[2] , 1 , boundaries[3])
...
def add_variables(data): 
    ...
    moving_window_1 , moving_window_2 , moving_window_3 = [] , [] , [] 
    ...
    for trial in range(ntrials):
        ...
        if trial != 0: 
            ...
            #(trial + 1) in indices: 
                ...
                loglikelihoods = np.log(stay_leave_decision(boundaries = parameter_set , current_task = df.loc[trial , "Chosen_castle"] , RAs = [df.loc[trial , "accuracy_1"] , df.loc[trial , "accuracy_2"] , df.loc[trial , "accuracy_3"]])) 
                
                #So we take the current castle - the previous one; if stay, then value is zero, if increment then 1, and if decrement, then -1
                #Then I added one to make them nice indices ranging from 0 to 2 to index the next step easily
                response = int(df.loc[trial , "Chosen_castle"] - df.loc[trial - 1 , "Chosen_castle"] + 1) 
...
column_list = ["Estimated_PE", "Estimated_LP" , "Estimated_Nov", "AIC" , "sub_id" , "n_trials" , "Accuracy_1"  , "Accuracy_2"  , "Accuracy_3"  , "mean_accuracy" , "BIC" , "negLL"]
...
if __name__ == '__main__':
    ...
    start_params = np.random.uniform(0 , 1) , np.random.uniform(0 , 1) , np.random.uniform(0 , 1) , np.random.uniform(0 , 1) 
    ...
    for file in filtered_filelist:
        ...
        estimated_data.loc[sub , ["sub_id" , "Estimated_PE" , "Estimated_LP" , "Estimated_Nov" , "AIC" , "BIC" , "negLL"]] = [sub , estimated_parameters[0] , estimated_parameters[1] , estimated_parameters[2] , AIC , BIC , LL]
...
estimated_data.to_csv("Fitting_results_participants_Testing.csv", columns = column_list, float_format ='%.3f')
