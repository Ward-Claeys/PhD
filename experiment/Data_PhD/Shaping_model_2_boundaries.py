...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 23 11:26:24 2026
    ...
    for trial in range(ntrials):
        ...
        if (trial != 0): #ntrials: 
            ...
            #(trial + 1) in indices: 
                ...
                loglikelihoods = np.log(stay_leave_decision(boundaries = parameter_set , current_task = df.loc[trial - 1 , "Chosen_castle"] , RAs = [df.loc[trial - 1 , "accuracy_1"] , df.loc[trial - 1 , "accuracy_2"] , df.loc[trial - 1 , "accuracy_3"]])) 
                ...
                response = int(np.sign(df.loc[trial  , "Chosen_castle"] - df.loc[trial - 1 , "Chosen_castle"]) + 1)
...
column_list = ["Boundary_Increment" , "Boundary_Decrement", "AIC" , "sub_id" , "n_trials" , "Accuracy_1"  , "Accuracy_2"  , "Accuracy_3"  , "mean_accuracy" , "BIC" , "negLL" , "slope"]
...
if __name__ == '__main__':
    ...
    filtered_filelist = [x for x in filtered_filelist if x != "Fitting_results_participants_Testing.csv"]
    filtered_filelist = [x for x in filtered_filelist if x != "Fitting_results_participants_Testing_2.csv"]
    filtered_filelist = [x for x in filtered_filelist if x != "TEST_Fitting_results_participants.csv"]
    filtered_filelist = [x for x in filtered_filelist if x[-19::] != "Fitting_results.csv"]
    filtered_filelist = [x for x in filtered_filelist if x[0:6] != "File_pp"]
    filtered_filelist = [x for x in filtered_filelist if x != "Fitting_results_participants_Testing_Fixed_Boundaries.csv"]
    filtered_filelist = [x for x in filtered_filelist if x != "Simulated_data.csv"]
    filtered_filelist = [x for x in filtered_filelist if x != "Recovery.csv"]
    filtered_filelist = [x for x in filtered_filelist if x != "Parameter_Estimation_4_Boundaries.csv"]
    filtered_filelist = [x for x in filtered_filelist if x != "Parameter_Estimation_Shared_Boundaries.csv"]
    ...
    for file in filtered_filelist:
        ...
        #data.to_csv("File_pp" + sub)
        
        n = data.shape[0]
        
        #Minimize negative loglikelihood
        optimization_output = optimize.minimize(fun = likelihood , x0 = start_params , args = "Fitting_results.csv" , options = {'maxfev':10000, 'xatol':0.00001, 'return_all':0} , method = "Nelder-Mead")# , verbose = 1)
        ...
        estimated_data.loc[sub , ["sub_id" , "Boundary_Increment" , "Boundary_Decrement" , "AIC" , "BIC" , "negLL" , "slope"]] = [sub , estimated_parameters[0] , estimated_parameters[1] , AIC , BIC , LL , estimated_parameters[2]]
        
        data = pd.read_csv(file)
        
        estimated_data.loc[sub , "Accuracy_1"] = np.mean(data.query("Chosen_castle == 4 & Number_of_doors == 2")["Accuracy"])
        estimated_data.loc[sub , "Accuracy_2"] = np.mean(data.query("Chosen_castle == 4 & Number_of_doors == 4")["Accuracy"])
        estimated_data.loc[sub , "Accuracy_3"] = np.mean(data.query("Chosen_castle == 4 & Number_of_doors == 8")["Accuracy"])
        estimated_data.loc[sub , "mean_accuracy"] = np.mean(data.query("Chosen_castle == 4")["Accuracy"])
        
        print("Simulated data")

    #Write results of parameter fitting
estimated_data.to_csv("Parameter_Estimation_Shared_Boundaries.csv", columns = column_list, float_format ='%.3f')
