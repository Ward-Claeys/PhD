...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 17 13:58:19 2026
...
from Model_simulation import generate_dataset
...
def stay_leave_decision(boundaries , current_task , RAs):
    ...
    if current_task == 1: 
        ...
        probability = sigmoid(RAs[0] , boundaries[4] , boundaries[0])
        ...
        increment = sigmoid(RAs[1] , boundaries[4] , boundaries[1])
        ...
        decrement_stay = sigmoid(RAs[1] , boundaries[4] , boundaries[2])
        ...
        #Get the normalized value for the decision
        probability = sigmoid(RAs[2] , boundaries[4] , boundaries[3])
...
column_list = ["simulation" , "Estimated_Boundary_Task_1", "Estimated_Boundary_high_Task_2" , "Estimated_Boundary_low_Task_2" , "Estimated_Boundary_Task_3" , "AIC" , "n_trials" , "BIC" , "negLL" , "Estimated_slope" , "Boundary_Task_1", "Boundary_high_Task_2" , "Boundary_low_Task_2" , "Boundary_Task_3" , "slope" , "n_trials_1" , "n_trials_2" , "n_trials_3"]
estimated_data = pd.DataFrame(columns=column_list)

column_list_2 = ["Current_task" , "Accuracy" , "Boundary_Task_1" , "Boundary_Task_2_low" , "Boundary_Task_2_high" , "Boundary_Task_3" , 
...
for simulation in range(n_simulations): 
    ...
    start_params = np.random.uniform(0 , 1) , np.random.uniform(0 , 1) , np.random.uniform(0 , 1) , np.random.uniform(0 , 1) , np.random.uniform(0 , 1) 
    ...
    estimated_data.loc[simulation , ["simulation" , "Estimated_Boundary_Task_1" , "Estimated_Boundary_high_Task_2" , "Estimated_Boundary_low_Task_2" , "Estimated_Boundary_Task_3" , "AIC" , "BIC" , "negLL" , "Estimated_slope"]] = [simulation , estimated_parameters[0] , estimated_parameters[1] , estimated_parameters[2] , estimated_parameters[3] , AIC , BIC , LL , estimated_parameters[4]]
    
    estimated_data.loc[simulation , ["Boundary_Task_1" , "Boundary_high_Task_2" , "Boundary_low_Task_2" , "Boundary_Task_3" , "slope"]] = [dataset.loc[1 , "Boundary_Task_1"] , dataset.loc[1 , "Boundary_Task_2_high"]  , dataset.loc[1 , "Boundary_Task_2_low"]  , dataset.loc[1 , "Boundary_Task_3"] , dataset.loc[1 , "slope"]]
...
estimated_data.to_csv("Recovery_4_boundaries.csv", columns = column_list, float_format ='%.3f')
