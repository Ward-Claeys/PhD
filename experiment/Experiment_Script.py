...
Trial_tracker   = visual.TextStim(win , pos = (0.7 , 0.8) , color = "white")
...
while (total_RT < n_seconds) and ((accuracy_1 < 0.7 or castle_1_count < 10) or (accuracy_2 < 0.7 or castle_2_count < 10) or (accuracy_3 < 0.7 or castle_3_count < 10)):
    ...
    #            Recties = visual.Rect(win , size = (0.4 , 0.6) , lineWidth = 20 , pos = positions_2[index] , color = "green" , fillColor = None)
        ...
        time_check = my_clock_2.getTime()
        Trial_tracker.text = "Minuten over: " + str(round(n_seconds // 60) - round(time_check // 60)) 
        Trial_tracker.draw()
    ...
    if Castle_choice == 1:
        ...
        while not incorrect and trial < 20:
            ...
            while numpy.sum(mouseResponses) == 0 or position == -9999:
                ...
                time_check = my_clock_2.getTime()
                Trial_tracker.text = "Minuten over: " + str(round(n_seconds // 60) - round(time_check // 60)) + "min"
                Trial_tracker.draw()
            ...
            trials[trial_nr - 1 , 7] = orientations[int(tr[random_indices[trial] , 0])]
            trials[trial_nr - 1 , 8] = int(tr[random_indices[trial] , 1])
            trials[trial_nr - 1 , 9] = size[int(tr[random_indices[trial] , 2])]
    ...
    total_RT = my_clock_2.getTime()
    
    print("********")
    print(total_RT)
    print(accuracy_1)
    print(accuracy_1 > 0.7)
    print(accuracy_2)
